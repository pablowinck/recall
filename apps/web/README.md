...
| `lib/`                | Shared hooks and helpers: Supabase Auth client and session, async actions, remote resources, appearance. |
...
- Import Supabase Auth only through `lib/supabase-auth.ts`, which keeps the storage key the full client used, so sessions survive.
